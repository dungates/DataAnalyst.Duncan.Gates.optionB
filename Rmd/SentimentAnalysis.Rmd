...
# Text size and color change
text_col <- ifelse(percentage_trigram$word %in% c("More Time To", "Nothing Nothing Nothing"), "black", "#324D56")
text_size <- ifelse(percentage_trigram$word %in% c("More Time To", "Nothing Nothing Nothing"), 15, 12) 

# Initial bar chart
(g5 <- ggplot(trigrams_united_graph, aes(reorder(word, n), n, fill = rev(factor(n)))) +
  geom_col(color = "black") +
  ggplot2::scale_fill_manual(values = c("#040404", "#04161D", "#042937", "#043B50", 
                                        "#044E6A", "#046084", "#04739D", "#0485B7", 
                                        "#0498D1", "#04ABEB")) +
  scale_y_continuous(breaks = scales::breaks_pretty(n = 8)) +
  geom_text(aes(label = n), hjust = -0.14, family = "Oswald") +
...
(pie5 <- ggplot2::ggplot(percentage_trigram, aes(x = 0, y = n, fill = reorder(word, n))) + #Pie chart input, ordered by n
  ...
  geom_text(aes(x = 0, y = 0, label = '0.28% of All Comments\nMention "More Time To"'), family = "Oswald", size = 3.8, fontface = "bold", vjust = 3.9) +
            scale_x_continuous(expand = c(1, 0)) + # Change x so expand is not default and adds no padding so the bars will produce a circle not a donut
            coord_polar(theta = "y", direction = -1) + # Make bars polar
            scale_fill_manual(values = c(rev(col(nrow(trigrams_united_graph))))) + # custom colors
            scale_color_manual(values = c(rev(col(nrow(trigrams_united_graph))))) +
            ggpubr::theme_transparent() +
            theme(legend.position = "none",
                  plot.title = element_text(hjust = 0.5, family = "Oswald"),
                  # plot.background = element_rect(fill = "#355c7d", color = "#355c7d")
