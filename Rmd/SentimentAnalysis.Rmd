...
theme_set(theme_minimal() + theme(plot.title = element_text(hjust = 0.5))) # General theme
...
GGally::ggcorr(data = factor_df, palette = "RdBu", label = T, label_size = 5, label_color = "white", hjust = 0.8, method = "pairwise") +
  geom_text(aes(label = "(Session Satisfaction)"), x = 1.8, y = 0.7)# Correlogram
```
...
text_additional_comments %>%
  ...
  coord_flip() +
  theme(legend.position = "none")
NAs <- c(NA, "na", "n/a", "no", "nope") # Different ways of saying no comment to filter out
text_additional_comments %>%
  count(word, sort = TRUE) %>%
  ...
  scale_fill_distiller(palette = "Blues") +
  geom_text(aes(label = n), hjust = -0.14) +
  labs(x = "", y = "", title = "Most Common Words for Additional Comments without NAs") + 
  coord_flip() +
  theme(legend.position = "none")
```
...
text_rating_reason %>%
  ...
  scale_fill_distiller(palette = "Blues") +
  geom_text(aes(label = n), hjust = -0.14) +
  labs(x = "", y = "", title = "Most Common Words for Rating Reason") + 
  coord_flip() +
  theme(legend.position = "none")
```
...
frequency <- text_df %>%
  ...
  ungroup() %>%
  select(-n) # Create a word frequency table still needs to take into account stopwards
...
```{r}
text_df %>%
  dplyr::filter(str_detect(text, "more time")) %>% # Filter only for "more time"
