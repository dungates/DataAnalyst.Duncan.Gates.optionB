...
gt_plots <- map_df(teaching_lab_df[cols_agree], ~ dplyr::recode_factor(., "Strongly agree" = 2,
  ...
  rename(`Clear\nFacilitation #1` = `S/he facilitated the content clearly....12`,
         ...
         Satisfaction = `I am satisfied with the overall quality of today's professional learning session.`,
         `Relevance to Role` = `Today's topic was relevant for my role.`,
  ...
  bind_cols(`Likeliness to Recommend to a Friend or Colleague` = teaching_df$likely_to_recommend_colleague_friend) %>% # Add other numeric column from df
  ...
  mutate(pos = case_when(likert_numeric >= 0 ~ 1))
  # nest(!c("Likeliness to Recommend to a Friend or Colleague", "Date"))

# Function for creating a plot
plot_group <- function(name, df) {
  plot_object <- ggplot(data = df, aes(x = factor(question), y = likert_numeric, fill = factor(pos, levels = c(1, 0)))) +
  geom_col(color = "black") +
  geom_text(aes(y = 0, label = ifelse(is.na(likert_numeric), "No Data Available", "")), family = "Oswald", color = "black") +
  scale_fill_manual(values = c("#04ABEB", "black")) +
  labs(y = "Score", x = "Question") +
  ggplot2::ylim(c(-2,2)) +
  theme_tl() +
  coord_flip() +
...
pandemic_response_table <- post_responses %>%
  ...
  rename(`Likert Plot (Blue = Agree, White = Disagree)` = plot) %>%
  select(-Unique, -data) %>%
  gt::gt() %>%
  fmt_ggplot(columns = vars(`Likert Plot (Blue = Agree, White = Disagree)`), height = px(275), aspect_ratio = 1) %>%
  tab_header(
    title = md("&#129440; **Reviews in the Post-Pandemic Era** &#129440;"),
    subtitle = md("Sorted *Worst-Best:* All 123 Responses")
  ) %>%
  ...
  cols_label(
    ...
    Date = md("**Date**"),
    `Select the name of your first facilitator.` = md("**Facilitator**"),
    `Likert Plot (Blue = Agree, White = Disagree)` = md("**Likert Plot (Blue = Agree, White = Disagree)**")
  ) %>%
  cols_align(
    align = "center",
    columns = vars(`How likely are you to recommend this professional learning to a colleague or friend?`)
