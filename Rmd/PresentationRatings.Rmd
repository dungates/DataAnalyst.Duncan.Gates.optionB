    ...
    locations = list(
      cells_body(
        columns = vars(`What could have improved your experience?`, `Why did you choose this rating?`, 
                       `Professional training session`, `Select the name of your first facilitator.`)
      )
    )
  ) %>%
  # tab_style(
...
## 123 Responses with ggplots of agree ratings!

```{r}
cols_agree <- c("I am satisfied with the overall quality of today's professional learning session.",
...
gt_plots <- map_df(teaching_lab_df[cols_agree], ~ dplyr::recode_factor(., "Strongly agree" = 2,
                       "Agree" = 1,
  ...
  rename(`Clear\nFacilitation #1` = `S/he facilitated the content clearly....12`,
         `Community of\nLearners #1` = `S/he effectively built a community of learners....13`,
         `Clear\nFacilitation #2` = `S/he facilitated the content clearly....16`,
         `Community\nof Learners #2` = `S/he effectively built a community of learners....17`,
         `Satisfied with\nOverall Quality` = `I am satisfied with the overall quality of today's professional learning session.`,
         `Relevance to\nRole` = `Today's topic was relevant for my role.`,
         `Helped Me\n Learn` = `The activities of today's session were well-designed to help me learn.`,
         `Apply Next 4-\n6 Weeks` = `How likely are you to apply this learning to your practice in the next 4-6 weeks?`) %>%
  bind_cols(`Likeliness to Recommend to a Friend or Colleague` = teaching_lab_df$`How likely are you to recommend this professional learning to a colleague or friend?`) %>% # Add other numeric column from df
  bind_cols(Date = teaching_lab_df$`Date for the session`) %>%
  # drop_na() %>%
  dplyr::filter(Date > "2020-04-01") %>%
  arrange(`Likeliness to Recommend to a Friend or Colleague`) %>%
  mutate(Unique = row_number()) %>% pivot_longer(names_to = "question", values_to = "likert_numeric", cols = !c("Date", "Likeliness to Recommend to a Friend or Colleague", "Unique")) %>%
  mutate(pos = case_when(likert_numeric >= 0 ~ "positive",
                         likert_numeric < 0 ~ "negative"))
  # nest(!c("Likeliness to Recommend to a Friend or Colleague", "Date"))
...
# Get sentiment for table
gt_sentiment <- sentimentr::sentiment(post_responses$`What could have improved your experience?`) %>% 
    group_by(element_id) %>%
    summarise(score = sum(sentiment)) %>%
  select(-element_id) %>%
  mutate(score = round(score, 2))

# Bind then relocate sentiment
post_responses <- post_responses %>%
  bind_cols(`AFINN Sentiment Score` = gt_sentiment$score) 
post_responses <- post_responses %>%
  relocate(`AFINN Sentiment Score`, .after = `What could have improved your experience?`) %>%
  select(-`Why did you choose this rating?`)
# Relocate all columns
post_responses <- post_responses[,c(4,5,6,1,2,3)]

# Table
pandemic_response_table <- post_responses %>%
  mutate(Unique = row_number()) %>%
  left_join(newdata, by = "Unique") %>%
  rename(`Likert Plot Scale` = plot) %>%
  select(-Unique, -data) %>%
  gt::gt() %>%
  fmt_ggplot(columns = vars(`Likert Plot Scale`), height = px(400), aspect_ratio = 1) %>%
  tab_header(
    title = md("&#129440; **Reviews in the Mid-Pandemic Era** &#129440;"),
    subtitle = md("Sorted *Worst-Best:* All 123 Responses")
  ) %>%
  cols_width(
    vars(`How likely are you to recommend this professional learning to a colleague or friend?`) ~ px(110),
    vars(`Likert Plot Scale`) ~ px(400),
    vars(`AFINN Sentiment Score`) ~ px(130),
         everything() ~ px(225)
  ) %>%
  cols_label(
    `How likely are you to recommend this professional learning to a colleague or friend?` = md("**Likeliness to Recommend**"),
    `What could have improved your experience?` = md("**What could have improved your experience?**"),
    `AFINN Sentiment Score` = md("**Positive/Negative Sentiment (-1 to 1)**"),
    `Professional training session` = md("**Professional training session**"),
    Date = md("**Date**"),
    `Select the name of your first facilitator.` = md("**Facilitator**"),
    `Likert Plot Scale` = md("**Likert Plot Ratings**")
  ) %>%
  cols_align(
    align = "center",
    columns = vars(`How likely are you to recommend this professional learning to a colleague or friend?`,
  ...
  tab_style(
    ...
    locations = list(
      cells_body(
        columns = vars(`Likert Plot Scale`)
      )
    )
  ) %>%
  tab_style(
  ...
  tab_style(
    ...
    locations = list(
      cells_body(
        columns = vars(`What could have improved your experience?`, `AFINN Sentiment Score`, 
                       `Professional training session`, `Select the name of your first facilitator.`,
                       `Date`)
      )
    )
  ...
  tab_options(
    ...
    stub.border.width = "1px",
    summary_row.border.color = "#989898")
gtsave(pandemic_response_table, here("HTML/PresentationTableggplot.html"))
```


## Filtered Ratings with ggplots
...
cols_filtered <- c("Clear\nFacilitation #1",
                ...
                "Helped Me\n Learn",
                "Apply Next 4-\n6 Weeks")
teaching_filtered_df <- teaching_lab_df %>%
  dplyr::filter(str_detect(`What could have improved your experience?`, c("more time|More time|collaborat")) == T) %>%
  dplyr::filter(`Date for the session` > "2020-04-01") %>%
  select("I am satisfied with the overall quality of today's professional learning session.",
                "Today's topic was relevant for my role.", 
                "The activities of today's session were well-designed to help me learn.",
                "How likely are you to apply this learning to your practice in the next 4-6 weeks?",
                "S/he facilitated the content clearly....12", 
...
gt_plots_filtered <- teaching_filtered_df %>% mutate_at(cols_filtered, ~ dplyr::recode_factor(., "Strongly agree" = 2,
                       "Agree" = 1,
                       "Neither agree nor disagree" = 0,
                       "Disagree" = -1,
                       "Strongly disagree" = -2)) %>% # Map across all agree based columns to change to 2,1,0,-1,2
...
newdata_filtered <- gt_plots_filtered %>%
  group_by(Unique) %>%
  nest() %>%
  mutate(plot = map2(Unique, data, plot_group))
...
pandemic_response_table_filtered <- post_responses %>%
  ...
  rename(`Likert Plot Scale` = plot) %>%
  select(-Unique, -data, -`AFINN Sentiment Score`) %>%
  gt::gt() %>%
  fmt_ggplot(columns = vars(`Likert Plot Scale`), height = px(400), aspect_ratio = 1) %>%
  tab_header(
    title = md("&#129440; **Reviews in the Mid-Pandemic Era** &#129440;"),
  ...
  tab_options(
    ...
    stub.border.width = "1px",
    summary_row.border.color = "#989898")
gtsave(pandemic_response_table_filtered, here("HTML/PresentationTableFilteredggplot.html"))
```
