...
## Plot by pre and post pandemic by EL training sessions dates.
ggplot(grouped_teaching_Guidebooks, aes(x = as.factor(`Date for the session`), y = score, 
...
ggplot(grouped_teaching_df, aes(x = `Date for the session`, 
                                ...
                                fill = as.factor(pro_grouping), 
                                color = factor(prepost, levels = c("Pre-Pandemic", "Post-Pandemic")))) + 
  geom_point(alpha = 0.3) +
  labs(x = "", y = "Likeliness to Recommend to a Friend or Colleague", title = "All Scores Comparison") +
  theme(legend.position = "none",
        plot.title = element_text(hjust = 0.5))
```
