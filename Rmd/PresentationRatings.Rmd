...
ggplot(grouped_teaching_df, aes(x = `Date for the session`,
  ...
  geom_col(color = ifelse(grouped_teaching_df$prepost == "Pre-Pandemic", "#324D56", "black")) +
...
ggsave(here("Images/PresentationAllScoresDate.png"), width = 14, height = 9)
```
...
(g_text <- 
  ...
  geom_jitter(size = 3, aes(alpha = prepost), height = 0.15, width = 0.28) + # Height changes width here since coord_flip already happened
    ...
    style = list(
      ...
      locations = cells_body(
        columns = vars(`Why did you choose this rating?`, 
         ...
         `Date`,
         `Select the name of your first facilitator.`),
        rows = `Professional training session` == "Guidebooks CM-6"
      )
    )
